...
import { Home, Search, Menu, X, BarChart3, User, LogOut, Flame, CreditCard } from 'lucide-react'
...
export default function Navigation() {
  ...
  try {
    ...
    // If context not available, default to false
  ...
  const allNavItems = [
    { href: '/', label: 'Home', icon: Home, hideForSubscribed: true },
    { href: '/patterns', label: 'Viral Database', icon: Search },
    { href: '/dashboard', label: 'Dashboard', icon: BarChart3 },
  ]
  
  const navItems = allNavItems.filter(item => 
    !(item.hideForSubscribed && hasActiveSubscription)
  )
    ...
    <nav className="bg-white border-b border-gray-200 sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16 relative">
          ...
          <div className="hidden md:flex md:items-center md:absolute md:left-1/2 md:top-0 md:h-full md:transform md:-translate-x-1/2">
              ...
              {navItems.map((item) => {
                ...
                return (
                  <Link
                    key={item.href}
                    href={item.href}
                    className={`inline-flex items-center px-1 h-full border-b-2 text-sm font-medium transition-colors relative ${
                      pathname === item.href
